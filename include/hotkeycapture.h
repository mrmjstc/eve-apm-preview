...
struct HotkeyCombination {
  ...
  bool operator==(const HotkeyCombination &other) const {
    return keyCode == other.keyCode && ctrl == other.ctrl && alt == other.alt &&
           shift == other.shift;
  }
};
...
public:
  ...
  int getKeyCode() const {
    return m_hotkeys.isEmpty() ? 0 : m_hotkeys.first().keyCode;
  }
  bool getCtrl() const {
    return m_hotkeys.isEmpty() ? false : m_hotkeys.first().ctrl;
  }
  bool getAlt() const {
    return m_hotkeys.isEmpty() ? false : m_hotkeys.first().alt;
  }
  bool getShift() const {
    return m_hotkeys.isEmpty() ? false : m_hotkeys.first().shift;
  }
