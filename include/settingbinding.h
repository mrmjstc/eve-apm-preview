...
public:
  ...
  SettingBinding(WidgetType *widget, Getter configGetter, Setter configSetter,
                 ValueType defaultValue, WidgetGetter widgetGetter,
                 WidgetSetter widgetSetter, Converter toWidget = nullptr,
                 Converter toConfig = nullptr)
      : m_widget(widget), m_configGetter(configGetter),
        m_configSetter(configSetter), m_defaultValue(defaultValue),
        m_widgetGetter(widgetGetter), m_widgetSetter(widgetSetter),
        m_toWidget(toWidget), m_toConfig(toConfig),
...
public:
  ...
  ColorButtonBinding(
      QPushButton *button, Getter configGetter, Setter configSetter,
      QColor defaultValue,
...
public:
  ...
  StringListTableBinding(QTableWidget *table, int column, Getter configGetter,
                         Setter configSetter, QStringList defaultValue);
...
public:
  ...
  CharacterHotkeyTableBinding(QTableWidget *table, Getter configGetter,
                              Setter configSetter);
...
public:
  ...
  HotkeyCaptureBinding(HotkeyCapture *widget, Getter configGetter,
                       Setter configSetter, HotkeyBinding defaultValue);
...
public:
  ...
  CycleGroupTableBinding(QTableWidget *table, Getter configGetter,
                         Setter configSetter, ButtonConnector buttonConnector);
...
public:
  ...
  FontBinding(QComboBox *fontCombo, QSpinBox *sizeSpinBox, Getter configGetter,
              Setter configSetter, QFont defaultValue);
...
inline std::unique_ptr<SettingBindingBase>
bindSpinBox(QSpinBox *widget, std::function<int()> getter,
            std::function<void(int)> setter, int defaultValue) {
  return std::make_unique<SettingBinding<QSpinBox, int>>(
      widget, getter, setter, defaultValue,
      [](QSpinBox *w) { return w->value(); },
      [](QSpinBox *w, int v) { w->setValue(v); });
}

inline std::unique_ptr<SettingBindingBase>
bindCheckBox(QCheckBox *widget, std::function<bool()> getter,
             std::function<void(bool)> setter, bool defaultValue) {
...
inline std::unique_ptr<SettingBindingBase>
bindComboBox(QComboBox *widget, std::function<int()> getter,
             std::function<void(int)> setter, int defaultValue) {
...
inline std::unique_ptr<SettingBindingBase>
bindSlider(QSlider *widget, std::function<int()> getter,
           std::function<void(int)> setter, int defaultValue) {
...
inline std::unique_ptr<ColorButtonBinding>
bindColorButton(QPushButton *button, std::function<QColor()> getter,
                std::function<void(QColor)> setter, QColor defaultValue,
                std::function<void(QPushButton *, const QColor &)> updateFunc) {
  return std::make_unique<ColorButtonBinding>(button, getter, setter,
                                              defaultValue, updateFunc);
}

inline std::unique_ptr<StringListTableBinding>
bindStringListTable(QTableWidget *table, int column,
                    std::function<QStringList()> getter,
                    std::function<void(const QStringList &)> setter,
                    QStringList defaultValue = QStringList()) {
  return std::make_unique<StringListTableBinding>(table, column, getter, setter,
                                                  defaultValue);
}

inline std::unique_ptr<CharacterHotkeyTableBinding> bindCharacterHotkeyTable(
    QTableWidget *table, std::function<QHash<QString, HotkeyBinding>()> getter,
    std::function<void(const QHash<QString, HotkeyBinding> &)> setter) {
  return std::make_unique<CharacterHotkeyTableBinding>(table, getter, setter);
}

inline std::unique_ptr<HotkeyCaptureBinding>
bindHotkeyCapture(HotkeyCapture *widget, std::function<HotkeyBinding()> getter,
                  std::function<void(const HotkeyBinding &)> setter,
                  HotkeyBinding defaultValue = HotkeyBinding()) {
  return std::make_unique<HotkeyCaptureBinding>(widget, getter, setter,
                                                defaultValue);
}

inline std::unique_ptr<CycleGroupTableBinding> bindCycleGroupTable(
    QTableWidget *table, std::function<QHash<QString, CycleGroup>()> getter,
    std::function<void(const QHash<QString, CycleGroup> &)> setter,
    std::function<void(QPushButton *)> buttonConnector) {
  return std::make_unique<CycleGroupTableBinding>(table, getter, setter,
                                                  buttonConnector);
}
...
inline std::unique_ptr<CharacterColorTableBinding> bindCharacterColorTable(
    ...
    std::function<void(QPushButton *)> buttonConnector) {
  return std::make_unique<CharacterColorTableBinding>(table, colorUpdateFunc,
                                                      buttonConnector);
}

inline std::unique_ptr<FontBinding>
bindFont(QComboBox *fontCombo, QSpinBox *sizeSpinBox,
         std::function<QFont()> getter,
         std::function<void(const QFont &)> setter, QFont defaultValue) {
  return std::make_unique<FontBinding>(fontCombo, sizeSpinBox, getter, setter,
                                       defaultValue);
}
